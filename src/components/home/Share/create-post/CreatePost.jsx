...
export const CreatePost = ({ close }) => {
    ...
    const uploadFile = (file) => {
        ...
        axios.post('http://localhost:5000/api/v1/user/post', {image : file , description}, {
            headers: {
                ...
                <div className="modal_body">
                    {
                        selectedImage ?
                            <div className='imageContainer'>
                                <textarea label="add a description" onChange={(e)=>setDescription(e.target.value)} value={description} placeholder='What do you want to talk about?' className='create_post_input' type="text" />
                                <img src={selectedImage} className="new_post_img" alt="" />
                            </div>
                            :
                            <>
                                <h1 onClick={togglePicker}>fasd</h1>
      {pickerOpen && <EmojiPicker onEmojiClick={(emoji) => console.log(emoji.emoji)} />}
                ...
                <div className="modal_bottom">
                    <div className="modal_actions">
                        ...
                        <button onClick={handleSubmit} className='btn_done' >Done</button>
                    </div>
                </div>
            </div>
        </>
